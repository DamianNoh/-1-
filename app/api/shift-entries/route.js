...
export async function GET(req) {
  ...
  if (date && workcenterId) {
    ...
    const [shiftCounts, overtime] = await Promise.all([
      prisma.shiftCount.findMany({ where: { date: d, workcenterId } }),
      prisma.overtimeEntry.findUnique({ where: { date_workcenterId: { date: d, workcenterId } } })
    ]);
  ...
  if (start && end) {
    ...
    const [shiftCounts, overtimeEntries] = await Promise.all([
      prisma.shiftCount.findMany({ where: { date: { gte, lte } }, include: { shiftType: true, workcenter: true } }),
      prisma.overtimeEntry.findMany({ where: { date: { gte, lte } }, include: { workcenter: true } })
    ]);
...
export async function POST(req) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const [shiftTypeId, headcount] of entries) {
      ...
      await tx.shiftCount.upsert({
        where: { date_workcenterId_shiftTypeId: { date: d, workcenterId, shiftTypeId } },
        update: { headcount: hc },
        create: { date: d, workcenterId, shiftTypeId, headcount: hc }
      });
    }
